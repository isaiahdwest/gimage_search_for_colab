...
# original code
ggl_srch = simple_image_download()
count = 0
try:
  image_urls = ggl_srch.urls('query', limit =20)
except:
  print('Error with image urls')
# download image
for i in image_urls:
    try:
      link = i
      filename = 'your-google-drive-path'+link.split("/")[-1]
      res = requests.get(link, stream=True)
      if res.status_code==200: # If we can access image via url
            res.raw.decode_content = True # returns stream content?
            with open(filename,'wb') as f: # opens a file with name=filename
                shutil.copyfileobj(res.raw, f) # saves image
      else:
          print('Image Couldn\'t be retreived')
    except:
      count+=1
print(f'There were {count} errors')
